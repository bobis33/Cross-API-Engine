...
namespace utl
{
    ...
    class Logger
    {
        ...
        public:
            ...
            template <typename Func> static void logExecutionTime(const std::string &message, const Func &&func)
            {
                const Clock clock;
                func();
                ...
                std::cout << getColorForDuration(duration)
                          ...
                          << LOG_LEVEL_COLOR.at(3);
            }
            static void logWarning(const std::string &message)
            {
                std::cout << LOG_LEVEL_COLOR.at(2) << formatLogMessage(LogLevel::WARNING, message)
                          << LOG_LEVEL_COLOR.at(3);
            }
            static void logInfo(const std::string &message)
            {
                std::cout << LOG_LEVEL_COLOR.at(1) << formatLogMessage(LogLevel::INFO, message)
                          << LOG_LEVEL_COLOR.at(3);
            }

        private:
            static constexpr std::array<const char *, 2> LOG_LEVEL_STRING = {"INFO", "WARNING"};
            static constexpr std::array<const char *, 4> LOG_LEVEL_COLOR = {"\033[31m", "\033[32m", "\033[33m",
                                                                            "\033[0m\n"};
            ...
            {
                return duration < 20.0F ? LOG_LEVEL_COLOR.at(1)
                                        : (duration < 90.0F ? LOG_LEVEL_COLOR.at(2) : LOG_LEVEL_COLOR.at(0));
            }
