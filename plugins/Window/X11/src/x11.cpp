...
bool cae::X11::create(const std::string &name, WindowSize size)
{
    ...
    g_ctx.window = XCreateSimpleWindow(g_ctx.display, root, 0, 0, size.width, size.height, 1,
    ...
    XSelectInput(g_ctx.display, g_ctx.window, ExposureMask | KeyPressMask | StructureNotifyMask);
...
void cae::X11::close()
{
    if (g_ctx.display != nullptr && g_ctx.window != 0U)
    {
...
bool cae::X11::shouldClose() const { return g_ctx.shouldClose; }

void cae::X11::pollEvents()
{
    while (XPending(g_ctx.display) != 0)
    {
        XEvent event;
        ...
        switch (event.type)
        {
            case Expose:
            {
                ...
                XColor color;
                const Colormap colormap = DefaultColormap(g_ctx.display, DefaultScreen(g_ctx.display));
                color.red = 0x0000;
                color.green = 0x0000;
                color.blue = 0x0000;
                ...
                XFillRectangle(g_ctx.display, g_ctx.window, gc, 0, 0, m_frameBufferSize.width,
            ...
            case ConfigureNotify:
                ...
                break;
            case ClientMessage:
                if (std::cmp_equal(event.xclient.data.l[0], g_ctx.wmDeleteMessage))
                {
